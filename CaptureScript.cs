...
public class CaptureScript : MonoBehaviour
{
	...
	IplImage skin;
	int avg_y = 100;//bCr顏色空間膚色cb的平均值
	int avg_cb_min = 80;//YCbCr顏色空間膚色cb的min值
	int avg_cb_max = 135;//YCbCr顏色空間膚色cb的max值
	int avg_cr_min = 131;//YCbCr顏色空間膚色cr的min值
	int avg_cr_max = 185;//YCbCr顏色空間膚色cr的max值
...
// Use this for initialization
		void Start ()
		{
			...
			if (devId >= 0) {
				...
				Scalar1 = new CvScalar(100, 131, 80);
				Scalar2 = new CvScalar(255, 185, 135);
				black = new CvScalar (0, 0, 0);
				white = new CvScalar (255, 255, 255);
			}
		}
